...
if echo "$PROMPT" | grep -qiE 'стройматериал|подбери материал|какие материал|кирпич|цемент|арматур|утеплител|гидроизол|штукатурк|плитк|ламинат|линолеум|бетон|поставщик|прораб|смет[аы] матери|сколько нужно'; then
  AGENT="materials-selector"
  REASON="задача связана с подбором стройматериалов"

# house-architect — генерация проектов домов
elif echo "$PROMPT" | grep -qiE 'проект дома|планировк[аи]|этаж|спальн|хавли|строительств|смет[аы]|чертёж|floor.?plan|house.?project|дом.*генер|генер.*дом|архитектор.*дом|кв\.?м|м²'; then
